...
const {
  notificationSchema,
} = require("../../../models/commonModels/notification");
  ...
  try {
    ...
    order.save().then(async (order) => {
      ...
      if (
        ...
        var user = await userSchema.findById(req.body.userId);
        if (user) {
          ...
          if (orders !== undefined && orders !== null) {
            ...
            user.orders = orders;
            const udpateUser = await userSchema.findByIdAndUpdate(
              req.body.userId,
              user
            );
          } else {
...
const deleteOrder = async (req, res) => {
  try {
    ...
    if (
      ...
      const orders = user.orders;
      const filtereredOrders = await orders.filter(
        (id) => id !== req.params.id
      );
      user.orders = orders;
...
const getOrdersInBulk = async (req, res) => {
  try {
    const data = await orderSchema.find({ _id: { $in: req.body.orderIds } });
